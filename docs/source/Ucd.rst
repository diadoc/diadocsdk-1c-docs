...
-  :doc:`GetContent <GetContent-(Ucd)>` - возвращает объект титула продавца УПД
-  :doc:`GetBuyerContent <GetBuyerContent-(Ucd)>` - возвращает объект титула покупателя УПД
-  :doc:`SendReceiptsAsync <SendReceiptsAsync-(Ucd)>` - формирует и подписывает документы по регламентному документообороту УПД
