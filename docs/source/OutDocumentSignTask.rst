...
- **Signer** (объект :doc:`Signer <Signer>`, чтение/запись) - данные подписанта документа. Используется для всех типов документов, кроме УПД. В случае, если производится подписание УПД, этот параметр игнорируется, вместо него используются подписанты, добавленные с помощью метода **AddExtendedSigner**.
