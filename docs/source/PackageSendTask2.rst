...
------
    ...
    -  :doc:`AddDocumentFromString <PackageSendTask2_AddDocumentFromString>` - добавляет документ в пакет на отправку, загружая его содержание из файла
...
3. Если отправка пакета с заполненным идентификатором операции завершилась успехам, то все остальные попытки отправки с тем же идентификатором не будут приводить к отправке нового пакета, а в результате выполнения метода вернется ссылка на ранее отправленный пакет.
