...
:DocflowStatus:
  ...
  Для понимания текущего статуса документа рекомендуется использовать поля, отвечающие за частичные статусы документооборота, например **SenderSignatureStatus**, **RecipientResponseStatus** и т.п.
