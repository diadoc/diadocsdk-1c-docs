...
:DocflowStatus:
  ...
  Поле рекомендуется использовать для отоборажения стаутса документа аналогично веб-интерфейсу Диадок.
  Для понимания текущего статуса документа рекомендуется использовать поля, отвечающие за частичные статусы документооборота, например **SenderSignatureStatus**, **RecipientResponseStatus*8 и т.п.
